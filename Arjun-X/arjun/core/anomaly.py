...
def get_dom_structure(html):
    """
    Extracts the HTML tag structure to create a structural fingerprint.
    """
...
def define(response_1, response_2, param, value, wordlist):
    ...
    factors = {
        ...
        'dom_hash': None # structural hash of the DOM
    }
    if type(response_1) == type(response_2) == requests.models.Response:
        ...
        # Structural Analysis
        hash1 = get_dom_structure(body_1)
        hash2 = get_dom_structure(body_2)
        if hash1 == hash2:
            factors['dom_hash'] = hash1
...
def compare(response, factors, params):
    ...
    # Structural Check
    if factors.get('dom_hash') is not None and get_dom_structure(response.text) != factors['dom_hash']:
        return ('dom structure', params, 'dom_hash')
