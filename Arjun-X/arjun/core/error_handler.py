...
def error_handler(response, factors):
	...
	if type(response) != str and response.status_code in (400, 413, 418, 429, 503):
		...
		elif type(response) == str and ('NameResolutionError' in response or 'Temporary failure' in response or 'Max retries exceeded' in response or 'Connection reset' in response):
			mem.var['dns_error_count'] = mem.var.get('dns_error_count', 0) + 1
			if mem.var['dns_error_count'] > 5:
				print('%s Too many connection errors, giving up.' % bad)
				return 'kill'
			print('%s Connection error, retrying...' % bad)
			time.sleep(2)
			return 'retry'
